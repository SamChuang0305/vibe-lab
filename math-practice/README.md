...
## 使用流程
1. 初次使用可先點擊設定頁的 `操作說明`，開啟 `user-guide.html` 查看完整新手教學。
2. 在設定頁依序調整題目種類、加、減法出題數字、乘、除法出題範圍、答題時間限制、秒數、題目數量、血條設定、積分顯示、音效。
3. 若要快速回到預設值，可點擊 `重置預設值`（位於 `開始練習` 右側）。
4. 點擊 `開始練習` 進入作答頁。
5. 輸入答案後點擊 `送出`，或直接按 `Enter` 送出（若開啟積分顯示，作答區右側會顯示目前積分）。
6. 全部題目完成（或提早離開）後進入結果頁查看統計、題目明細與學習建議。
7. 可點擊 `重新開始` 回到設定頁開始新一輪。
